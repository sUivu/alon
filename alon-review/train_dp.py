#!/usr/bin/env python
"""
ALON training script (ALONGNN).
Copied from alon and adapted for ALON experiments.

This script trains the ALONGNN model which DOES use mu_param (lambda) terms.
ALON uses augmented Lagrangian method with dual variable updates.
"""
...
from model.models import DecoupledAutogradLayer, ALONGNN_Network
...
def featurize_batch(args, batch):
    """Featurize a batch of graphs for ALONGNN."""
...
def construct_model(args, mu_param):
    """Construct ALONGNN model (needs mu_param for augmented Lagrangian)."""
...
def validate(args, model, val_loader, problem, mu_param=None):
    """Validate the ALONGNN model (with mu_param)."""
    total_loss = 0.
    total_score = 0.
    total_constraint = 0.
    total_count = 0
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            # ALONGNN: uses mu_param (lambda term)
...
def train(args, model, train_loader, optimizer, problem, val_loader=None, test_loader=None, mu_param=None):
    """
    Main training loop for ALONGNN (with mu_param updates).
    
    Unlike LiftMP, ALONGNN uses augmented Lagrangian method:
    ...
    while args.stepwise or ep <= epochs:
        ...
        for batch in train_loader:
            ...
            # ALONGNN: uses mu_param to compute the augmented Lagrangian loss
            x_out = model(x_in, batch, mu_param)
        ...
        # This is the key difference from LiftMP - ALONGNN updates dual variables
        if not ep % args.lambda_freq:
            with torch.no_grad():
                ...
                # Paper Eq. (10) / Alg. 1 line 7: mu <- [mu + rho*c]_+
                # (project the SUM, so signed residuals may lower mu).
                # Here c is the epoch sum over batches (add_mu_param) and the
                # step is lambda_ratio -- the existing +AL convention.  The
                # paper's mean-batch form divides that sum by n_batches.
                # --dual_clamp_first selects the clamp-first variant mu += rho*[c]_+.
                if getattr(args, 'dual_clamp_first', False):
                    add = add_mu_param.clamp(min=0.0)
                else:
                    add = add_mu_param
                mu_param += args.lambda_ratio * add
