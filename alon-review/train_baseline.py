...
Copied from bespoke-gnn4do-main and adapted for PDGNO experiments.

This script trains the LiftMP model which does NOT use lambda terms.
Lambda_param is passed as None, so only gradient information is used.
"""
...
# 获取当前脚本所在目录，并定位到项目根目录（假设根目录是当前脚本的上两层）
PROJECT_ROOT = Path(__file__).resolve().parent.parent
# 将项目根目录添加到 sys.path 的开头
sys.path.insert(0, str(PROJECT_ROOT))
print(PROJECT_ROOT)
from optgnn7.bespoke_gnn4do.model.models import AutogradLayer, LiftLayer, LiftNetwork
from optgnn7.bespoke_gnn4do.problem.problems import get_problem
from optgnn7.bespoke_gnn4do.data.loader import construct_loaders
import sys
...
def construct_model(args):
    """Construct LiftMP model (no lambda_param needed)."""
...
def validate(args, model, val_loader, problem):
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            # LiftMP: lambda_param=None (不使用lambda项)
...
def train(args, model, train_loader, optimizer, problem, val_loader=None, test_loader=None):
    """Main training loop for LiftMP (no lambda updates)."""
    epochs = args.epochs
    ...
    while args.stepwise or ep <= epochs:
        ...
        for batch in train_loader:
            ...
            # LiftMP: lambda_param=None (不使用lambda项，只用梯度信息)
            x_out = model(x_in, batch)
