#!/usr/bin/env python3
"""PDGNO upgrade campaign -- train one (arm, dataset, seed) FROM SCRATCH and
evaluate under the corrected protocol in the same process.

Arms D0-D4 see model/pdgno_upgrade.py.  Everything except the named mechanism
changes is identical across arms:
  ...
  * 200 epochs, lr 1e-3, grad_clip 5, penalty annealing -0.5 -> 2.5 linear
    (the released ALMGNN schedule), batch size per dataset (PROTEINS 9,
...
Isolated namespace: writes only ckpts/pdgno_upgrade_* and results/pdgno_upgrade/.

Usage
-----
  CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=2 \
  /app/pdgno-env/bin/python train_pdgno_upgrade.py \
...
from model.pdgno_a_fair import (  # noqa: E402
    ...
    node_violation, PDGNOA_Fair,
)
from model.pdgno_upgrade import (  # noqa: E402
...
def main():
    ...
    tag = f'pdgno_upgrade_{cli.arm}_{cli.dataset}_s{cli.seed}'
    save_dir = cli.save_dir or os.path.join('ckpts', tag)
    os.makedirs(save_dir, exist_ok=True)
    os.makedirs('results/pdgno_upgrade', exist_ok=True)
    ...
    out_path = f'results/pdgno_upgrade/eval_{cli.arm}_{cli.dataset}' \
