...
# [MODIFIED] fixed generic ALM dataset preprocessing function
def prepare_alon_dataset(dataset):
    """
    Inject global edge indices into any PyG dataset. Supports GeneratedDataset,
    TUDataset/PPI, etc.
    """
    # 1. first check the custom-generated total_num_nodes attribute
    if hasattr(dataset, 'total_num_nodes'):
        return dataset, dataset.total_num_nodes

    global_node_counter = 0
    new_data_list = []

    print(f"Pre-processing dataset: {len(dataset)} graphs found.")

    for i in range(len(dataset)):
        # force materialization of the Data object (this handles TUDataset's slices logic)
        data = dataset[i].clone()


        num_constraints = data.num_nodes  # the number of constraints now equals the number of nodes

        # inject the global index mapping
        data.node_global_idx = torch.arange(
            global_node_counter,
            global_node_counter + num_constraints,
            dtype=torch.long
        )
    ...
    # [CRITICAL FIX] manually overwrite PyG's internal data list so the
    # DataLoader can read the new attribute
    # PyG internally uses _data_list to store materialized objects
    dataset._data_list = new_data_list

    # also, for compatibility, dynamically inject total_num_nodes if the
    # original object lacks it
    dataset.total_num_nodes = global_node_counter

    return dataset, global_node_counter
...
def construct_loaders(args, mode=None):
    ...
    mu_param = torch.zeros(total_nodes, device=args.device)
    # print("total num nodes in dataset:", num_nodes_total)
    # mu_param = torch.nn.Parameter(
    #     torch.zeros(num_nodes_total, device=args.device)
    #     )
    ...
    if mode == "test":
        ...
        return train_loader, val_loader, test_loader, mu_param
    else:
        raise ValueError(f"Invalid mode passed into construct_loaders: {mode}")
