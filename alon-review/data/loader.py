...
# [MODIFIED] 修复后的通用 ALM 数据集预处理函数
def prepare_alm_dataset(dataset):
    """
    为任何 PyG 数据集注入全局边索引。支持 GeneratedDataset 和 TUDataset/PPI 等。
    """
    # 1. 优先检查自定义生成的 total_num_nodes 属性
    ...
    for i in range(len(dataset)):
        # 强制获取 Data 对象（这会处理 TUDataset 的 slices 逻辑）
        data = dataset[i].clone() 
        

        num_constraints = data.num_nodes  # 约束数量现在等于节点数量
    ...
    # [CRITICAL FIX] 手动覆盖 PyG 的内部数据列表，确保 DataLoader 能读到新属性
    # PyG 内部使用 _data_list 存储物化后的对象
    dataset._data_list = new_data_list
    
    # 同时为了兼容性，如果原对象没有 total_num_nodes，我们动态注入它
