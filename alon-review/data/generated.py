...
class GeneratedDataset(InMemoryDataset):
    ...
    def __init__(self, root, name, generator,
        ...
        self.total_num_nodes = obj["total_num_nodes"]
    ...
    def process(self):
        ...
        if self.pre_transform is not None:
            _data_list = [self.pre_transform(data) for data in data_list]
        data_list = _data_list
    # Change 2: core modification, switch from counting edges to counting nodes
        global_node_counter = 0
        for data in data_list:
            num_nodes = data.num_nodes
            # Assign a globally unique index to each node in the graph for precise
            # updates of the dual variables mu later on
            data.node_global_idx = torch.arange(
                global_node_counter,
                global_node_counter + num_nodes,
                dtype=torch.long
            )
            global_node_counter += num_nodes
        ...
        torch.save(
                {
                    "data": data,
                    "slices": slices,
                    "total_num_nodes": global_node_counter, # store total node count
                },
