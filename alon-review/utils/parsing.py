...
def add_train_args(parser: ArgumentParser):
    ...
    parser.add_argument('--model_type', type=str, default='LiftMP', choices=['LiftMP', "ALONGNN", "ALONGNN_V2", 'GIN',"ALONGIN","ALONGatedGCNN","ALONGAT","ALONGCNN", 'GAT', 'GCNN', 'GatedGCNN'],
    ...
    parser.add_argument('--num_layers_project', type=int, default=4,
                        help='How many projection layers?')
    parser.add_argument('--rank', type=int, default=32,
                        help='How many dimensions for the vectors at each node, i.e. what rank is the solution matrix?')
    parser.add_argument('--dropout', type=float, default=0.1,
                        help='Model dropout')
    parser.add_argument('--lift_ratio', type=float, default=0.2,
                        help='Ratio for lift layers')
    ...
    parser.add_argument('--valid_freq', type=int, default=25,
    ...
    parser.add_argument('--lambda_freq', type=int, default=25,
                        help='Save model every N steps/epochs (0 to only save at end of training)')
    parser.add_argument('--penalty', type=float, default=0.25,
                        help='Penalty for constraint violations')
    parser.add_argument('--penalty_s', type=float, default=0.25,
                        help='Penalty for constraint violations')
    parser.add_argument('--penalty_e', type=float, default=0.25,
                        help='Penalty for constraint violations')
    parser.add_argument('--lambda_ratio', type=float, default=0.5,
                        help='Penalty for lambda constraint violations ratio compared to main constraint violations')
    parser.add_argument('--dual_clamp_first', action='store_true',
                        help='Alternative dual update mu += lambda_ratio*[c]_+ (clamp the residual '
                             'before the step); default is mu <- [mu + lambda_ratio*c]_+.')
    parser.add_argument('--mu_cap', type=float, default=None,
                        help='If set, clamp mu_param to this upper bound after each dual update (RB over-regularization fix)')
    parser.add_argument('--dual_warmup', type=int, default=0,
                        help='Skip dual (mu) updates for epochs < this value (embedding warm-up)')
    ...
    parser.add_argument('--stepwise', action='store_true',
                        help='Train by number of gradient steps (default: epoch-based)')
    ...
    parser.set_defaults(stepwise=False)
def hash_dict(d):
...
def parse_train_args() -> Namespace:
    ...
    parser.add_argument('--gurobi_timeout', type=float, default=60.0,
                        help='Timeout for Gurobi')
