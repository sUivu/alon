#!/usr/bin/env python3
"""
PDGNO-A: Amortized-Warmstart PDGNO for Minimum Vertex Cover.

NON-INVASIVE addition to the PDGNO codebase.  Nothing in the existing files
(model/models.py, model/training.py, problem/*.py, data/loader.py, train*.py)
is modified: this module only IMPORTS and reuses them.

Motivation
----------
The existing ALMGNN (PDGNO) keeps a per-node free dual ``mu_param`` of length
...
PDGNO-A fixes this by REPLACING the free per-node dual with:
...
training):
    ...
    x      <- ALM_theta( x_in, graph, mu )           # frozen base network
...
Author: PDGNO-A proof-of-concept.
"""
...
from model.models import ALMGNN_Network, DecoupledAutogradLayer  # noqa: F401
from problem.problems import get_problem  # noqa: F401
from problem.losses import get_vertex_cover_violations  # noqa: F401
from data.loader import construct_dataset, prepare_alm_dataset  # noqa: F401
...
# ============================================================================
# 2) PDGNO-A wrapper: amortized init + explicit primal-dual ascent
# ============================================================================
class PDGNOA(nn.Module):
    """Wraps a frozen/live ALMGNN and adds an amortized dual + ascent loop.

    Forward equations (as implemented):

        x0        = ALM_theta(x_in, G, mu=0)                  # dual-free embedding
        mu0       = softplus(mu_phi([x0, struct_feats]))      # amortized init, >=0
        x         = ALM_theta(x_in, G, mu0)                   # warm-started primal
        for k in 1..K:                                        # explicit dual ascent
            c_i   = sum_{j in N(i)} phi_ij(x)                 # node residual
            mu_i  = max(0, mu_i + rho * c_i)
            x     = ALM_theta(x_in, G, mu)                    # re-solve primal
...
def round_repair(V, n, edges, seed=0, n_hyperplanes=1000, penalty=1.0):
    """Randomized-hyperplane rounding (same protocol as PDGNO's
...
def build_base_model(args, device):
    """Construct the untouched ALMGNN base (reusing model.models)."""
    problem = get_problem(args)
    grad_layer = DecoupledAutogradLayer(problem=problem)
    model = ALMGNN_Network(grad_layer=grad_layer, in_channels=args.rank,
