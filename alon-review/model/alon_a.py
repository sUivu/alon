...
# ============================================================================
def greedy_repair_cover(n, edges, cover, mode='greedy'):
    """Repair a (possibly infeasible) vertex cover until every edge is covered.

    ``mode='greedy'`` (paper rule, default): repeatedly add the vertex that
    covers the MOST currently-uncovered edges, breaking ties by lowest index.
    This mirrors scripts/reeval_main_feasible.py::greedy_repair exactly and is
    the single repair rule shared by all evaluated arms.

    ``mode='degree'``: alternative single pass adding, for each uncovered edge,
    its higher-degree endpoint (lowest-index tie-break).
    """
    cover = set(cover)
    if mode == 'degree':
        deg = [0] * n
...
def round_repair(V, n, edges, seed=0, n_hyperplanes=1000, penalty=1.0,
    ...
    then GREEDY REPAIR of any uncovered edge using the paper rule (vertex
    covering the most uncovered edges, lowest-index tie-break; see
    greedy_repair_cover).  ``repair='degree'`` selects the degree-based variant.
    ...
    for u, v in edges:
        if u not in cover and v not in cover:
            raw_uncovered += 1
    cset = greedy_repair_cover(n, edges, cover, mode=repair)
    return {
        'raw_size': raw_size,
