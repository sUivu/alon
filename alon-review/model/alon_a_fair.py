...
from model.alon_a import (  # noqa: E402,F401
    ...
    _graph_edges, greedy_repair_cover,
)
...
# so their coefficients are renormalised by the factors below to keep the
# un-halved convention consistent.
...
class ALONA_Fair(nn.Module):
    ...
    def forward(self, x_in, batch, K=0, rho=0.1, use_mu_phi=True,
        ...
        # unchanged, so 2/PHI gives the reference step scale.
...
def round_repair_batch(V, graphs, offsets, seed=0, n_hyperplanes=1000,
                       penalty=1.0, verify=True, repair='greedy'):
    ...
    the paper rule (vertex covering the most currently-uncovered edges,
    lowest-index tie-break) exactly as model.alon_a.greedy_repair_cover and
    scripts/reeval_main_feasible.py::greedy_repair.  ``repair='degree'``
    selects the degree-based variant.
    """
    N, r = V.shape
