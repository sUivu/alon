    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            with torch.enable_grad():
                ...
                # 正确计算 loss 对输入 x_in 的梯度
                grad_x = torch.autograd.grad(loss_grad, x_in_grad, create_graph=False)[0]  # 形状与 x_in 相同

                for p, p_snap in zip(model.parameters(), param_snapshot):
                    assert torch.allclose(p, p_snap), "模型参数被修改！"
                # 将 grad_x 调整为二维矩阵以便 SVD（如果原维度 >2）
                if grad_x.dim() > 2:
                    grad_x_2d = grad_x.view(grad_x.size(0), -1)
                else:
                    grad_x_2d = grad_x
                U, S, V = torch.svd(grad_x_2d)          # 奇异值降序排列
                min_sv = S[-1].item()                    # 最小奇异值

                # 计算标量内积 (grad_x 与 x_in_grad 逐元素相乘后求和)
                ...
                opt__ += add_opt__   # 注意 total_score 的用法
                eigen += min_sv
                # print("add_opt_:", add_opt_,"add_opt__:", add_opt__, "add_gap:", add_gap, "eigen:", eigen)   # 注意 total_score 的用法
