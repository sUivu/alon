#!/usr/bin/env python3
"""PDGNO upgrade campaign -- mechanism/parameter arms D0-D4 (isolated namespace).

Extends the fair bake-off (model/pdgno_a_fair.py, imported read-only) with the
...
actually runs within a 200-epoch budget (H3):
  ...
  D0 'baseline'            : per-node norm + signed residual + released dual
                             ...
                             fires at 200 epochs).  Reference PDGNO.
...
from model.pdgno_a_fair import (  # noqa: E402,F401
...
def dual_step(mu_param, add_mu, n_batches, schedule, rho_t, c_mean_prev,
    ...
    residuals are unchanged, so 2/PHI restores the historical step scale.
