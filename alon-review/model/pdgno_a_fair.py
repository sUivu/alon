#!/usr/bin/env python3
"""
PDGNO-A FAIR bake-off module (isolated namespace).
...
in ``model.pdgno_a``.  It does not touch any other worker's files.
"""
import math
import sys
...
from model.pdgno_a import (  # noqa: E402,F401
...
def build_network(candidate, problem, rank, num_layers, lift_ratio,
    ...
    net = FairALMGNN(grad, in_channels=rank, num_layers=num_layers,
...
# ----------------------------------------------------------------------------
# PDGNO-A wrapper: amortized dual init + K explicit ascent steps
# ----------------------------------------------------------------------------
...
class PDGNOA_Fair(nn.Module):
    """Wraps a FairALMGNN with an amortized dual initializer and explicit
    primal-dual ascent.
...
def round_repair_batch(V, graphs, offsets, seed=0, n_hyperplanes=1000,
    ...
    """Vectorized EXACT equivalent of model.pdgno_a.round_repair per graph.
...
@torch.no_grad()
def eval_config(pdgno, x_in, big, graphs, offsets, K, rho, use_mu_phi=True,
                adaptive=True, delta=0.25, tau=2.0, rho_max=None,
    ...
    if statusquo:
        ...
        x, c = pdgno.net_forward(x_in, big, zero)
        mu_norm_traj, resid_traj = [0.0], [float(c.norm())]
        mu = None
    else:
        x, info = pdgno(x_in, big, K=K, rho=rho, use_mu_phi=use_mu_phi,
