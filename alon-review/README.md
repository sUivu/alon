...
updated by projected dual-ascent steps during training,
`mu <- [mu + rho * c]_+` (paper Eq. (10) / Alg. 1 line 7), under an annealed
**positive** penalty coefficient `rho > 0`.
...
remaining uncovered edges are fixed by the paper's **greedy repair**
(repeatedly add the vertex covering the most uncovered edges, lowest-index
tie-break). The reported quantity is always the size of the *repaired,
feasible* cover (`violation == 0` is asserted per graph).
...
*statusquo* run (`mu=0`, 1000 hyperplanes, seed 0, paper greedy repair) whose
`mean_repaired` is the reported number. The penalty coefficient rho is
annealed linearly and kept positive, `+0.5 -> +2.5` (the paper requires
`rho > 0`).
...
# (positive penalty schedule; the paper requires rho > 0.)
...
seed 0), greedy-repair uncovered edges with the paper rule (add the vertex
covering the most currently-uncovered edges, lowest-index tie-break; shared by
all arms — `model/alon_a.greedy_repair_cover`, mirrored by
`scripts/reeval_main_feasible.py::greedy_repair`), and report the repaired
cover size; `violation == 0` is asserted per graph. Duals are updated during
training by the paper's projected form `mu <- [mu + rho * c]_+`, where `c` is
the signed node violation accumulated over the epoch's minibatches (the paper
presents full-batch steps, so `rho` absorbs the batch count).
