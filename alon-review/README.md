...
The paper's method is called **ALON**. In the code it appears under historical
class names: `ALMGNN_Network` (in `model/models.py`, trained by
`train.py`/`train_dp.py`) and `PDGNOA_Fair` (in `model/pdgno_a_fair.py`, the
model used by the Table-1 harness `scripts/matrix_fill/train_t1.py`).

## Repository layout

```
train.py                  raw-backbone training (GIN/GAT/GCNN/GatedGCNN)
train_dp.py               ALM-variant training (ALMGIN/ALMGAT/ALMGCNN/ALMGatedGCNN)
train_pdgno_upgrade.py    shared dataset specs / helpers for the ALON harness
train_baseline.py         OptGNN/LiftMP baseline (requires optgnn7, see DATA)
...
scripts/
  ...
  eval_pdgno_a_fair.py              single-checkpoint protocol evaluation
...
results/classical_baselines/ENZYMES.json   reference optima used by the smoke test
...
- **OptGNN baseline (`train_baseline.py`)** imports the `optgnn7` package,
  which is *not* bundled here. Obtain it from the original public OptGNN
  repository and place it at the repository root. ALON and raw-backbone
  training/evaluation do not need it.
...
CUDA_VISIBLE_DEVICES= python scripts/eval_pdgno_a_fair.py \
    --checkpoint ckpts/pdgno_a_fair_C0_ENZYMES_seed0_screen_ep200/pdgno_a_fair.pt \
...
for k in sorted(exp):
    ...
    print(f"{k:10s} expected={a} got={b} {'OK' if a == b else 'MISMATCH'}")
EOF
...
for split in ('test',):
    for cfg in exp['splits'][split]:
        e, g = exp['splits'][split][cfg], got['splits'][split][cfg]
        print(cfg, 'rep_mean', e['rep_mean'], '==', g['rep_mean'],
              'OK' if e['rep_mean'] == g['rep_mean'] else 'MISMATCH')
EOF
```
