...
REUSES scripts/reeval_main_feasible.py (imported, NOT modified):
  ...
  - ALM/PDGNO cells evaluated with mu = 0 (paper protocol)
OptGNN (model_type LiftMP, trained by train_baseline.py) uses the same protocol
with the optgnn7 LiftNetwork branch (mirrors run_supplementary_ood in
reeval_main_feasible.py).

Only writes under results/retrain_campaign/ - results/reeval_main/ untouched.
Run: CUDA_VISIBLE_DEVICES=2 /app/pdgno-env/bin/python \
...
# optgnn7 (the OptGNN reference codebase) is NOT bundled with this package;
# fetch it from the original public OptGNN repository (see README) if you need
# to evaluate OptGNN/LiftMP checkpoints. ALON checkpoint evaluation works
# without it.
try:
    from optgnn7.bespoke_gnn4do.model.models import AutogradLayer as OptGNN_AutogradLayer, LiftNetwork  # noqa: E402
    from optgnn7.bespoke_gnn4do.problem.problems import get_problem as get_optgnn_problem  # noqa: E402
    HAVE_OPTGNN7 = True
except ImportError:
    OptGNN_AutogradLayer = LiftNetwork = None
    get_optgnn_problem = None
    HAVE_OPTGNN7 = False


def eval_optgnn_checkpoint(ckpt_path, params, device):
    """LiftMP/OptGNN eval under the identical protocol (paper split, rounding
    seed 0 / 1000 hyperplanes, greedy repair)."""
    args = R.make_args(params, device)
    torch.manual_seed(0)
    torch.cuda.manual_seed_all(0)

    dataset = construct_dataset(args)
    dataset, _ = prepare_alm_dataset(dataset)
    n = len(dataset)
...
def evaluate_job(job, device):
    ...
    rec = {
        ...
        'mu_at_eval': 0 if params['model_type'] == 'ALMGNN' else 'n/a (no dual variable)',
