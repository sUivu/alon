#!/usr/bin/env python3
"""Overnight W1: train ONE (arm, dataset, seed) FROM SCRATCH, isolated
namespace (ckpts/dual_overnight_*, results/dual_overnight/).

Training loop = verbatim upgrade-campaign loop (train_pdgno_upgrade.py /
scripts/dual_tuning/train_sweep.py semantics): 200 epochs, lr 1e-3,
grad_clip 5, penalty anneal -0.5 -> 2.5, paper split, alg1 dual schedule
with the arm's lambda_freq / rho / tau / mu_mode / warmup.

Arms: E1-E4 (scripts/dual_overnight/overnight_configs.py) or any dual-tuning
T-config name for transfers.  Eval: test-split statusquo (mu=0) rounding
seed 0 / 1000 hyperplanes -> greedy repair (violation==0 asserted) +
asc_K5_r0.02 secondary run.

Usage:
  CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=2 \
  /app/pdgno-env/bin/python scripts/dual_overnight/train_arm.py \
      --arm E1 --dataset PROTEINS --seed 0
"""
import argparse
import json
import os
...
from data.loader import construct_dataset, prepare_alm_dataset  # noqa: E402
from model.pdgno_a_fair import (  # noqa: E402
    PDGNOA_Fair, make_batch_graphs, edge_residual, node_violation,
    round_repair_batch, eval_config, summarize, _PHI, _PHI2,
)
from model.pdgno_a import build_split_graphs  # noqa: E402
from train_pdgno_upgrade import (  # noqa: E402
...
def main():
    ...
    tag = f'dual_overnight_{args.arm}_{args.dataset}_s{args.seed}'
    save_dir = os.path.join('ckpts', tag)
    os.makedirs(save_dir, exist_ok=True)
    os.makedirs('results/dual_overnight', exist_ok=True)
    out_path = (f'results/dual_overnight/eval_{args.arm}_{args.dataset}'
                f'_s{args.seed}.json')
    ...
    tinfo = {}
    if args.skip_train:
        ck = torch.load(os.path.join(save_dir, 'pdgno_upgrade.pt'),
        ...
        net = net.to(device)
        tinfo = ck['train_info']
        log(f'[skip_train] loaded {save_dir}/pdgno_upgrade.pt')
    else:
        net = build_overnight_network(cfg, problem, rank=ns.rank,
                                      num_layers=ns.num_layers,
                                      lift_ratio=ns.lift_ratio,
        ...
        for ep in range(args.epochs):
            ...
            for bi, blist in enumerate(batches):
                ...
                with torch.no_grad():
                    ...
                    ep_viol += float(c.clamp(min=0).mean()) / _PHI[cfg['residual']]
        ...
        torch.save({'arm': args.arm, 'cfg': cfg, 'dataset': args.dataset,
                   ...
                   os.path.join(save_dir, 'pdgno_upgrade.pt'))
        log(f'[saved] {save_dir}/pdgno_upgrade.pt')
        net.eval()
    ...
    pdgno = PDGNOA_Fair(net, problem, rank=ns.rank).to(device)
    pdgno.eval()
    split_out = {'n_test_graphs': len(graphs), 'runs': []}

    def run(name, K, rho, statusquo=False):
        rows, mh, rh = eval_config(
            pdgno, x_in, big, graphs, offsets, K, rho, use_mu_phi=False,
