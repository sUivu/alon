...
Extends model/pdgno_upgrade.py + train_pdgno_upgrade.py READ-ONLY (imports
...
Usage:
  ...
  /app/pdgno-env/bin/python scripts/dual_tuning/train_sweep.py \
...
from model.pdgno_upgrade import UpgradeALMGNN  # noqa: E402
...
def build_tuned_network(cfg, problem, rank, num_layers, lift_ratio,
    ...
    return UpgradeALMGNN(grad, in_channels=rank, num_layers=num_layers,
...
# ----------------------------------------------------------------------------
# Tunable dual step (extension of model.pdgno_upgrade.dual_step alg1 branch)
# ----------------------------------------------------------------------------
...
def main():
    ...
    out = {
        ...
        'checkpoint': os.path.join(save_dir, 'pdgno_upgrade.pt'),
