...
Paper-aligned defaults: penalty schedule is POSITIVE (+0.5 -> +2.5;
paper requires rho > 0), and
the dual update is the projected sum mu <- [mu + rho_t*mean_batch(c)]_+
(paper Eq. (10) / Alg. 1 line 7).

Eval (identical to campaign primary metric): test split, mu=0 statusquo,
rounding seed 0 / 1000 hyperplanes -> paper greedy repair (add the vertex
covering the most currently-uncovered edges, lowest-index tie-break;
feasibility asserted); report mean repaired size + infeasible-before-repair.
...
def tuned_dual_step(mu_param, add_mu, n_batches, cfg, rho_t, c_mean_prev,
                    mu_prev):
    # add_mu is un-halved; /PHI gives the reference c_mean value, and
    # 2/PHI gives the reference multiplier-step scale for product residuals.
